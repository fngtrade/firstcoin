...
reference the release tarball available from https://firstcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcoin git repository](https://github.com/firstcoin/firstcoin) but are not part of
...
Currently only `Source10` is used, for the example `firstcoin.conf` file.
...
in the firstcoin git repository. At present only an SVG version of the Bitcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the firstcoin
...
The `firstcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `firstcoin-server` package. At this point, most RPM based
...
The `firstcoin-server` package should have SELinux support. How to properly do
...
[Ringing Liberty](https://www.ringingliberty.com/firstcoin/). He has been
...
4. I renamed the `firstcoin` package that contains the Qt GUI to `firstcoin-core` as
that appears to be how the general population refers to it, in contrast to
`firstcoin-xt` or `firstcoin-classic`. I wanted to make sure the general population
